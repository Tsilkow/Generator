...
bool TileMap::load(hexSettings& hSetts, mapSettings& mSetts, std::map<Coords, Hex>& hexes)
{
    ...
    std::vector<sf::Color> drainageGradient;

    createGradient(heightGradient, sf::Color(255, 255, 255), sf::Color(191, 191, 191), 6);
    createGradient(elevationGradient, sf::Color(64, 255, 64), sf::Color(255, 255, 255), 6);
    createGradient(temperatureGradient, sf::Color(255, 255, 255), sf::Color(255, 128, 0), 10);
    createGradient(moistureGradient, sf::Color(255, 255, 255), sf::Color(0, 128, 255), 100);
    createGradient(drainageGradient, sf::Color(0, 128, 96), sf::Color(255, 255, 255), 100);
	 
    if(!m_tileset.loadFromFile(mSetts.filename)) return false;
    m_texturedMap.setPrimitiveType(sf::Quads);
    ...
    m_drainageMap.setPrimitiveType(sf::Quads);
	...
	{
	    // map colored based on hex moisture
	    m_drainageMap.append(
		   sf::Vertex(position[i], drainageGradient[std::max(0, it->second.getDrainage())],
			      blankPos[i]));
	}
...
void TileMap::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
    ...
    switch(m_mode)
    {
	case DisplayMode::Textured: target.draw(m_texturedMap, states); break;
	...
	case DisplayMode::Drainage: target.draw(m_drainageMap, states); break;
    }
}
